...
class QBLexer(Lexer):
    ...
    # NUMBER = r'\d+'  # See multiple regex patterns in @_ for NUMBER instead.
    ...
    # Ignored pattern
    # ignore_REM = r'^\s*[Rr][Ee][Mm]\w+.*\n'  # See def REM instead.
    # ignore_REM = r'[Rr][Ee][Mm]\w+.*\n'  # See def REM instead.
    ignore_SQ = r"'.*\n"
    ...
    def __init__(self):
        ...
        self.expression_stack = []  # horizontal nesting
        self.enders = {
            '(': ")",
        }
        # 'DEFSTR': r'$', (N/A for _CAST)
        self._CAST = {
            '%': np.int16,  # QB Integer
    ...
    @_(r'[rR][eE][mM]')
    def REM(self, t):
        print("[REM] IGNORED COMMENT")
        # no return (ignore the comment)
        pass

    def _matching(self, opener):
        if isinstance(opener, str):
            return self.enders[opener]
        elif isinstance(opener, (list, tuple)):
            return [self.enders[key] for key in opener]
        else:
    ...
    def on_newline(self):
        if self.expression_stack:
            raise SyntaxError(
                "{}, line {}: line ended before {}"
                .format(self.path, self.lineno,
                        self._matching(self.expression_stack)))
    ...
    def newline(self, t):
        self.lineno += t.value.count('\n')
        print("[QBLexer] [newline] count={}".format(t.value.count('\n')))
        self.on_newline()

    # @_(r"'.*\n")
    def ignore_SQ(self, t):
        # no return (ignore comment)
        pass
    ...
    def rparen(self, t):
        t.type = ")"
        if self.expression_stack:
            raise SyntaxError("{}, line {}: Got extra {}"
                              .format(self.path, self.lineno, t.value))
        ender = self.enders[self.expression_stack.pop()]
        if ender != t.value:
            raise SyntaxError("{}, line {}: Got {} before {}".format(t.value)
                              .format(self.path, self.lineno, t.value, ender))
        return t
    ...
    def NUMBER(self, t):
        t.type = 'LITERAL'
        _cast = self._CAST.get(t.value[-1:])
        if t.value.startswith('&H'):
            t.value = np.int32(t.value[2:], 16)
        elif t.value.startswith('&O'):
            t.value = np.int32(t.value[2:], 8)
        elif t.value.startswith('&B'):
            t.value = np.int32(t.value[2:], 2)
        if _cast is not None:
            t.value = _cast(t.value[:-1])
            # ^ stop before -1 since ends with type suffix in this case
        elif "." in t.value:
            t.value = np.float64(t.value)
        else:
            t.value = np.int32(t.value)
        return t

    # Based on https://regex101.com/r/rX4rL7/1
    # found via Google
    @_(r'"[^"]*"')
    def STRING_LITERAL(self, t):
        t.type = 'LITERAL'
        if not isinstance(t.value, str):
            raise NotImplementedError("oops, cast to str here")
        t.value = t.value[1:-1]
        return t
    # region generated by qbstatic
    ...
    def error(self, t):
        raise SyntaxError("{}, line {} Illegal character '{}'"
                          .format(self.path, self.lineno, t.value[0]))
